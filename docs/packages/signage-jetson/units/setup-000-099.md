...
## **085_enable_led_gpio24.sh**

このスクリプトは **Raspberry Pi 系ボードのみ**実行され、ブート時に **GPIO24（BCM24, 物理ピン18）を出力・High** に初期化します。  
...
!!! note "補足"
    ...
    - Low 始動にしたい場合は gpio=24=op,dl（drive-low）に変更してください。

---

## **086_install_gpio3_shutdown_service.sh**

このスクリプトは **Raspberry Pi 系ボードのみ**実行され、GPIO3（**BCM 3 / 物理ピン 5**）の**長押しシャットダウン**ハンドラを有効にします。  
実体の Python スクリプトを所定パスへ配置（リンク/コピー）し、systemd ユニットを作成して **自動起動**させます。`BOARD_TYPE` に `rasp` / `pi` を含まない場合は安全にスキップします。

> 生成／変更されるファイル

- **/usr/local/bin/gpio3_shutdown.py**  
  - ソースファイル: `scripts/power/gpio3_shutdown.py`  
  - 配置方法: `install_or_link` により **シンボリックリンク or コピー**（冪等）
- **/etc/systemd/system/gpio3-shutdown.service**（systemd ユニット）

!!! note "補足"
    - **I²C との競合**: GPIO3 は I²C1 の SDA ピンです。I²C を使用する構成では、本機能と競合する可能性があります。
